    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-sm">
          ...
          <div className="flex items-center">
            ...
            <p className="text-2xl font-bold text-slate-900">{data.summary.totalProducts}</p>
          </div>
        </div>
        <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-sm">
          ...
          <div className="flex items-center">
            ...
            <p className="text-2xl font-bold text-slate-900">{data.summary.totalStockUnits}</p>
          </div>
        </div>
        <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-sm">
          ...
          <div className="flex items-center">
            ...
            <p className="text-2xl font-bold text-slate-900">{data.summary.lowStockProducts}</p>
          </div>
        </div>
        <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-sm">
          ...
          <div className="flex items-center">
            ...
            <p className="text-2xl font-bold text-slate-900">{data.summary.outOfStockProducts}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-sm flex flex-wrap gap-4 items-end">
        ...
        <div>
          ...
          <select value={stockStatus} onChange={e => setStockStatus(e.target.value)} className="block w-full border border-slate-300 rounded-md py-2 px-3 text-sm">
            ...
            <table className="min-w-full divide-y divide-slate-200">
              ...
              <tbody className="bg-white divide-y divide-slate-200">
                {data.data.map((p: any) => {
                  ...
                  return (
                    <tr key={p.id}>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-900">{p.productName}</td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-slate-900 text-right">{p.currentStock}</td>
